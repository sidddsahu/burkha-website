...
const Customer = () => {
  ...
  const generatePDF = () => {
    ...
    doc.setFontSize(12);
    doc.text(
      `Order #${state.selectedOrder._id.slice(-6).toUpperCase()}`,
      140,
      54
    );
    doc.setFontSize(10);
    ...
    doc.text(
      `Payment: ${state.selectedOrder.paymentStatus.toUpperCase()}`,
      140,
      74
    );
    ...
    doc.setFontSize(10);
    doc.text(
      `Subtotal: ₹${state.selectedOrder.totalPrice.toFixed(2)}`,
      14,
      finalY
    );
    if (state.selectedOrder.totalPriceAfterDiscount) {
      doc.text(
        `After Discount: ₹${state.selectedOrder.totalPriceAfterDiscount.toFixed(
          2
        )}`,
        14,
        finalY + 6
  ...
  const columns = [
...

    {
      ...
      cell: (row) => (
        <button
          onClick={() =>
            navigateToVendor(row.orderItems[0]?.discountName?.address)
          }
          className='text-blue-600 hover:text-blue-800 hover:underline text-left'
        >
    ...
    },
    {
      ...
      selector: (row) =>
        `₹${
          row.dueAmount ??
          (row.totalPriceAfterDiscount ?? row.totalPrice) -
            (row.paidAmount || 0)
        }`,
      sortable: true,
      width: "120px",
